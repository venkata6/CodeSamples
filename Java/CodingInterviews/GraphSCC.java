...
public class GraphSCC
{
     ...
     */
    public GraphSCC(int v)
    {
	    number_of_vertices=v;
	    graph=new Graph(number_of_vertices);
	    graphRev=new Graph(number_of_vertices);
	    stack=new ArrayDeque<Integer>(v);
    }
    
    public static void DFS ( Graph g, int index) {
    ...
    public static int  secondDFS ( Graph g, int index) {
    	if ( g.visited.get(index) ) {
    ...
    public static void main(String...arg)
    {
        ...
        try
        {
            ...
            Graph finalGraph = new Graph(noOfVertices);
            for ( int i=1; i <= graphSCC.graph.adjacencyList.size(); i++){
            	List<Integer> edges = graphSCC.graph.adjacencyList.get(i);
            	//System.out.println(" original graph " + i  + " --> "+  Arrays.toString(edges.toArray()));
            	
            	List<Integer> edgesNew= finalGraph.getEdge(graphSCC.graphRev.finishingTimes.get(i));
            	for ( int j : edges){
